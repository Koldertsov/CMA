...
import KRobust_CBS.CBS_KRobust_CMA;
...
import LargeAgents_CBS.Solvers.HighLevel.CBS_LargeAgents_CMA;
...
//                new CBS_Shapes(),
                new CBS_LargeAgents(), new CBS_LargeAgents_CMA()
        };
        String problemType = "LargeAgents";


        String[] folders = new String[]{
//                "empty-32-32-large-agents-2x2",
//                "empty-32-32-large-agents-3x3",
//                "empty-32-32-large-agents-4x4",
//                "empty-32-32-large-agents-5x5",
//                "1616-2x2",
//                "1616-3x3",
//                "1616-4x4",
                "1616-5x5",
        ...
        for (String folder : folders) {
            for (I_Solver solver : solvers) {
                largeAgentRunManager = new RunManager_ExperimentsLA(folder, solver, new int[]{4, 6});
                largeAgentRunManager.runAllExperiments();
            }
            String experimentName = problemType + " " + folder + " Date - ";
    ...
    public static void runExperiments_Robust() {
        ...
        I_Solver[] solvers = new I_Solver[]{
               new CBS_KRobust(), new CBS_KRobust_CMA()
...
//                "empty-8-8", "empty-16-16", "empty-32-32",
//                "random-32-32-10", "random-32-32-20"
                "empty-32-32"
        };
        ...
        for (int k = 2; k < 3; k++) {
            for (String folder : folders) {
                for (I_Solver solver : solvers) {
                    robustRunManager = new RunManager_ExperimentsKRobust(folder, solver, k, new int[]{6, 8, 10, 12, 14});
                    robustRunManager.runAllExperiments();
                }
                String experimentName = problemType + " " + folder + " k=" + k;
                outputResults(experimentName);
            }
        }
    }
