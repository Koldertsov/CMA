...
//    }
    ...
    public void prepareInstances(String instanceName, InstanceManager.InstancePath instancePath, InstanceProperties instanceProperties) {
        ...
        /*
         * Previous implementation read the scenario file once and distributed its
         * lines across all requested agent-counts.  That meant the file had to
         * contain the *sum* of all requested counts, otherwise later instances
         * were created with too few (or zero) agents.
         *
         * We now re-read the scenario file for *each* requested agent-count, so
         * every instance always takes the first <requestedAgents> lines of the
         * .scen file.  This matches the user’s expectation that, e.g., running
         * with {8,10,12} only requires 12 lines in the scenario file.
         */

        for (int requestedAgents : numOfAgentsFromProperties) {
            ArrayList<String> agentLines = getAgentLines(moving_ai_path, requestedAgents);

            Agent[] agents = getAgents(agentLines, requestedAgents);

            if (instanceName == null || agents == null || agents.length == 0) {
                continue; /* Invalid parameters – skip this instance */
            }

            I_Map graphMap = getMap(moving_ai_path, instanceProperties);
            if(graphMap == null){ continue; }
            MAPF_Instance curInstance = new MAPF_Instance(instanceName, graphMap, agents);
            curInstance.setObstaclePercentage(instanceProperties.obstacles.getReportPercentage());
            this.instanceList.add(curInstance);
        }
    }
