...
public class Solution implements Iterable<SingleAgentPlan>{
    ...
    public Iterator<SingleAgentPlan> iterator() {
        List<Agent> agents = new ArrayList<>(this.agentPlans.keySet());
        Collections.sort(agents, Comparator.comparingInt(a -> a.iD));
        List<SingleAgentPlan> orderedPlans = new ArrayList<>(agents.size());
        for (Agent agent : agents) {
            orderedPlans.add(this.agentPlans.get(agent));
        }
        return orderedPlans.iterator();
    }
    ...
    public Spliterator<SingleAgentPlan> spliterator() {
        List<SingleAgentPlan> ordered = new ArrayList<>();
        this.forEach(ordered::add);
