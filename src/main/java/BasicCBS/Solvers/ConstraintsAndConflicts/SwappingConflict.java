...
public class SwappingConflict extends A_Conflict{
    ...
    public SwappingConflict(Agent agent1, Agent agent2, int time, I_Location agent1_destination, I_Location agent2_destination) {
        super(
                (agent1.iD <= agent2.iD ? agent1 : agent2),
                (agent1.iD <= agent2.iD ? agent2 : agent1),
                time,
                (agent1.iD <= agent2.iD ? agent1_destination : agent2_destination)
        );
        this.agent2_destination = (agent1.iD <= agent2.iD ? agent2_destination : agent1_destination);
    }
    ...
    public static A_Conflict conflictBetween(Move move1, Move move2){
        if(SwappingConflict.haveConflicts(move1, move2)){
            Agent a1 = move1.agent;
            Agent a2 = move2.agent;
            if(a1.iD <= a2.iD){
                return new SwappingConflict(a1, a2, move1.timeNow, move1.currLocation, move2.currLocation);
            } else {
                return new SwappingConflict(a2, a1, move1.timeNow, move2.currLocation, move1.currLocation);
            }
        }
        else{
            return null;
