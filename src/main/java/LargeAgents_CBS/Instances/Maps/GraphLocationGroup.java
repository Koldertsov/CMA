...
public class GraphLocationGroup implements I_Location {
    ...
    private Queue<GraphMapVertex_LargeAgents> outerCells = new PriorityQueue<>(new Comparator<GraphMapVertex_LargeAgents>() {
        ...
        public int compare(GraphMapVertex_LargeAgents cell_1, GraphMapVertex_LargeAgents cell_2) {
            int cmp = Integer.compare(cell_1.getNeighbors().size(), cell_2.getNeighbors().size());
            if(cmp != 0){
                return cmp; // fewer neighbors first
            }
            // deterministic tie-breaker by coordinate
            int x1 = ((BasicCBS.Instances.Maps.Coordinates.Coordinate_2D)cell_1.getCoordinate()).x_value;
            int y1 = ((BasicCBS.Instances.Maps.Coordinates.Coordinate_2D)cell_1.getCoordinate()).y_value;
            int x2 = ((BasicCBS.Instances.Maps.Coordinates.Coordinate_2D)cell_2.getCoordinate()).x_value;
            int y2 = ((BasicCBS.Instances.Maps.Coordinates.Coordinate_2D)cell_2.getCoordinate()).y_value;
    ...
    public List<I_Location> getNeighbors() {
        ...
        GraphMapVertex_LargeAgents minNeighborsCell = this.outerCells.peek(); // deterministic: don't remove from the copied list order depends on PQ iterator
        outerCellsList.remove(minNeighborsCell);
...
//        this.intersectWithRefPoint((GraphLocationGroup) other);
    ...
    public int hashCode() {
        ...
        for (int i = 0; i < this.mapCells.length; i++) {
            for (int j = 0; j < this.mapCells[i].length; j++) {
                GraphMapVertex_LargeAgents e = this.mapCells[i][j];
